...
def run_car(simulator: Simulator) -> None:
    ...
    if abs(new_steering)<np.pi*0.5/9:
        new_steering = new_steering*0.2
    if abs(steering)>np.pi*0.5/5:
        is_curve = True
    else:
        is_curve = False

    if len(dt_list)>100:
        dt_mean = (np.min(dt_list)+np.max(dt_list))/2
        normalized_dt=(dt-np.mean(dt_list))/np.std(dt_list)
        print("dt",dt,"dt_mean",dt_mean,"normalized_dt",normalized_dt)
        throttle+=throttle*normalized_dt
        dt_list.pop(0)
