...
def stabilize_steering_angle(
    ...
    last_steering_angle = last_steering_angle*180/math.pi
    new_steering_angle = new_steering_angle*180/math.pi
    """
    ...
    angle_deviation = new_steering_angle - last_steering_angle
    if abs(angle_deviation) > max_angle_deviation:
        stabilized_steering_angle = int(last_steering_angle + max_angle_deviation * angle_deviation / abs(angle_deviation))
    else:
        stabilized_steering_angle = new_steering_angle
    return stabilized_steering_angle*math.pi/180
...
def run_car(simulator: Simulator) -> None:
    ...
    if abs(new_steering)>np.pi*0.5/6:
    ...
    sinc = 1.3*math.sin(math.sin(2*math.pi*new_steering)/(2*math.pi*new_steering))**2
    throttle = 7*sinc
    if throttle>7:
        throttle = 7
    if throttle<2:
