...
def get_slope(image,degree = 2):
    ...
    #print("num_pixels",num_pixels,"num_components",num_components)
    if num_components < 2:
        return last_output
    if num_components > 2:
      if np.all(np.sort(num_pixels)[::-1][:2]>1000):
        ...
        y_min = max([np.min(first_edge.nonzero()[0]),np.min(second_edge.nonzero()[0])])
        
        y1_max = np.max(first_edge.nonzero()[0])
        y2_max = np.max(second_edge.nonzero()[0])
        x1_max = int(np.polyval(first_poly,y1_max))
        x2_max = int(np.polyval(second_poly,y2_max))

        return first_poly,second_poly,slope,[x0,y0],steering_angle,2,y_min,x1_max,x2_max
    ...
    y_min = np.min((edge.nonzero()[0]))

    y_max = np.max((edge.nonzero()[0]))
    x_max = int(np.polyval(poly,y_max))

    return poly,poly,slope,[x0,y0],steering_angle,1,y_min,x_max,x_max
...
def run_car(simulator: Simulator) -> None:
    ...
    first_poly,second_poly,slope,point,steering_angle,number_lanes,y_min,x1_max,x2_max = last_output
    ...
    out_img = cv2.circle(out_img.copy(), (int(point[0]),point[1]), radius=5, color=(0, 0, 255), thickness=3)
    out_img = cv2.line(out_img.copy(), (out_img.shape[1],y_min), (0,y_min), (0, 255, 0), thickness=3)
    out_img = cv2.line(out_img.copy(), (x1_max,out_img.shape[0]), (x1_max,0), (255, 0, 0), thickness=3)
    out_img = cv2.line(out_img.copy(), (x2_max,out_img.shape[0]), (x2_max,0), (255, 255, 0), thickness=3)
    ...
    confidence=(520-y_min)/200
    if confidence>1.2:
        throttle*=3
    else:
        throttle*=confidence
    #throttle = confidence**2 * throttle
    #if throttle>10:
    #    throttle = 25
    #if throttle<4:
    #    throttle = 4
    #if confidence<0.5:
    #    throttle=4
    #throttle = confidence * 15
    print("steering",steering*180/np.pi,"throttle",throttle,"confidence",confidence)
...
if __name__ == "__main__":
    ...
    judge = Judge(team_code="XXHFOprCe", zip_file_path="L:\Roro\college\senior2\STPMacathon40\simulation\Machathon4.0-Judge\Team_XXHFOprCe.zip")
