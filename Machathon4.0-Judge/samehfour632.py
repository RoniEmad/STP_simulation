...
def run_car(simulator: Simulator) -> None:
    ...
    factor = np.sin(2*(steering/(np.pi*0.5/4))**2)
    # factor=4*(steering/(np.pi*0.5/4.5))**2#1-math.log10(2*dsteering+1.4)
    new_steering = abs(factor)*steering*0.95
    ...
    throttle = 15*(1-math.log10(10*abs(factor)+1))
    ...
    simulator.set_car_velocity(throttle)#*1.1)
