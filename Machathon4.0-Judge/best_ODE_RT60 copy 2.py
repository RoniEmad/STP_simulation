...
dummymodel = torchvision.models.resnet18(pretrained=True)
...
def run_car(simulator: Simulator) -> None:
    ...
    global Tensorizer
    global device
    resized = resizer.forward((Image.fromarray(img)))
    tensorized = Tensorizer(resized)
    tensorized = tensorized.unsqueeze(0)
    output = dummymodel(tensorized.cuda())
    print(output)
