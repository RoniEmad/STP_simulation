...
def get_slope(image,degree = 2):
    ...
    if "Left" in lanes.keys() and "Right" in lanes.keys():
        ...
        y3 = np.max(edge.nonzero()[0])-10
        x3 = np.polyval(poly,y3)
        print(available)
        if available == "Right" and x3 > image.shape[1]/2:
            steering_angle = 0.85*np.pi
        elif available == "Left" and x3 < image.shape[1]/2:
...
def run_car(simulator: Simulator) -> None:
    global last_output,last_steering_angle,acc,first_frame
    # if first_frame:
    #     simulator.set_car_velocity(0)
    #     simulator.set_car_steering(0)
    #     first_frame = False
    #     time.sleep(3)
    """
    ...
    throttle = 15*(1-math.log10(15*abs(factor)+1))
    ...
    if keyboard.is_pressed("w") or keyboard.is_pressed("up"):
        ...
        throttle = -4
    actual_steering = steering * simulator.max_steer_angle / 1.5
    actual_throttle = throttle * 7

    simulator.set_car_steering(actual_steering)
    simulator.set_car_velocity(actual_throttle)
