...
def run_car(simulator: Simulator) -> None:
    global last_output,last_steering_angle#,last_time,dt_list,is_curve,time_now
    #if is_curve:
    #    min_dt=100
    #else:
    #    min_dt=10
    ...
    steering = (0.5*np.pi-steering_angle)/5
    ...
    factor=4*(steering/(np.pi*0.5/5))**2#1-math.log10(2*dsteering+1.4)
    ...
    sinc = 1.79*math.sin(math.sin(2*math.pi*new_steering)/(2*math.pi*new_steering))**2
    throttle = 7*sinc-1
    ...
    simulator.set_car_steering(new_steering*0.9)
    simulator.set_car_velocity(throttle*1.35)#*(dt/60))
